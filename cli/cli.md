...
```bash
aws s3 --endpoint-url https://w3s.storage mb s3://my-bucket
```

## List buckets

```bash
aws s3 --endpoint-url https://w3s.storage ls
```

## List objects

```bash
aws s3 --endpoint-url https://w3s.storage ls s3://bucket-name
```

## Upload object

```bash
aws s3 --endpoint-url https://w3s.storage cp /path/to/file s3://bucket-name
```

## Get object

```bash
aws s3 --endpoint-url https://w3s.storage cp s3://bucket-name/file /path/to/file
```

## Delete object

```bash
aws s3 --endpoint-url https://w3s.storage rm s3://bucket-name/file
```
...
```bash
aws s3 --endpoint-url https://w3s.storage rm s3://bucket-name/ --recursive
```

## Delete bucket

```bash
aws s3 --endpoint-url https://w3s.storage rb s3://bucket-name
```

## Copy object

```bash
aws s3 --endpoint-url https://w3s.storage cp s3://bucket-name/file s3://bucket-name/file
```
